...
* **Complejidad Temporal:** $O(N \log N)$ para ambos algoritmos.
  * $N$ es el número total de nodos (intersecciones) en el mapa.
  * **¿De dónde sale?** Insertar y extraer todos los vértices de la cola de prioridad (min-heap) toma $O(\log N)$ por cada elemento. Al hacerlo para $N$ elementos a lo largo del proceso de búsqueda, nos da el famoso tiempo cuasilineal $O(N \log N)$.
...
* **Complejidad Espacial:** $O(N)$
  * Utilizamos una **lista de adyacencia** (implementada como diccionarios anidados de Python) que guarda los nodos y aristas con un uso de memoria directamente proporcional a $N$. Esto es inmensamente más eficiente que usar una Matriz de Adyacencia $O(N^2)$. 
  * **¡Punto clave para defender!** Ahora que procesamos el dataset real de Medellín con **27,000 nodos**, una matriz de adyacencia requeriría $27,000 \times 27,000 = 729,000,000$ (729 millones) de celdas en memoria, lo cual congelaría la RAM de la computadora. En cambio, nuestra lista de adyacencia procesa esto en fracciones de segundo porque el grafo de calles es extremadamente "disperso" (las calles no se conectan todas con todas).
