...
* **Complejidad Temporal:** $O((V + E) \log V)$ para ambos algoritmos.
  ...
  * Utilizamos una **lista de adyacencia** (implementada como diccionarios anidados de Python) que guarda los $V$ nodos y $E$ aristas. Esto es inmensamente más eficiente que usar una Matriz de Adyacencia $O(V^2)$. 
