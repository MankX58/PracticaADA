...
### 6. Parseo de Strings Seguros (`ast.literal_eval`)
**Dónde lo usamos:** Al leer las coordenadas del dataset real en `datos.py`.
```python
import ast
tup = ast.literal_eval("(-75.572, 6.211)")
```
**¿Qué significa?**
El nuevo dataset de 68,000 calles nos entrega los nodos como textos literales: `"(-75.572, 6.211)"`. Para Python, eso es solo una palabra. Necesitábamos convertirlo en una tupla real de números `(lon, lat)` para poder hacer matemáticas con ellos. 
En lugar de usar métodos peligrosos o manuales para "recortar" paréntesis, la función `literal_eval` del módulo `ast` (Abstract Syntax Trees) lee el texto y si reconoce que tiene forma de estructura de datos válida (una tupla), la transforma mágicamente a código Python de manera 100% segura.

### 7. "Mapeo Espacial" (Spatial Matching) en `obtener_nodo_real_cercano()`
**Dónde lo usamos:** En la interfaz web, para que los algoritmos funcionen.
**¿Qué significa?**
El usuario selecciona un barrio amistoso ("Poblado") en la interfaz web, pero ¡el grafo de 27,000 nodos no sabe qué es "Poblado"!, solo conoce coordenadas como `(-75.57, 6.21)`.
Para solucionar esto, creamos una función que barre los 27,000 nodos calculando la fórmula de Distancia Haversine contra las coordenadas teóricas del "Poblado". El nodo que arroje la distancia mínima es seleccionado como el punto de inicio real en la malla vial. ¡Es como arrojar un alfiler en Google Maps y que el sistema "se enganche" a la calle más cercana!
